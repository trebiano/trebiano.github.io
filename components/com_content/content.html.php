	...
	*/
	function showContentList( $title, &$items, &$access, $id=0, $sectionid=NULL, $gid, &$params, &$pageNav, $other_categories, &$lists, $order, $categories_exist ) {
		...
		if ( $params->get( 'page_title' ) ) {
			...
			<h1><?php echo htmlspecialchars( $title->name, ENT_QUOTES ); ?></h1>
			</div>
			<?php
		}
		?>
		...
		if ( $params->get( 'item_title' ) ) {
			if ( $params->get( 'link_titles' ) && $row->link_on != '' ) {
				...
				<td class="contentheading<?php echo $params->get( 'pageclass_sfx' ); ?>" width="100%">
					<a href="<?php echo $row->link_on;?>" class="contentpagetitle<?php echo $params->get( 'pageclass_sfx' ); ?>" title="<?php echo $row->title;?>">
