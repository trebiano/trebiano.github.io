...
/**
 * IT IS A THE CONTENT OF freecap14.php I DO NOT KNOW HOW TO REFERENCE THE ORIGINAL PHP FILE 
 * 
 * cedric walter
 */
/************************************************************\
...
//only change required to let freecap run in Joomla security images plugins 
include ('../../../../../configuration.php');
require ($mosConfig_absolute_path . "/administrator/components/com_securityimages/pluginsA/freecap/1.4/config.php");
//end changes www.waltercedric.com

session_name( md5( $mosConfig_live_site ) );
session_start();
	
$_SESSION['hash_func'] = $hash_func;
$bg_type = (isset ($_GET['trans'])) ? 0 : rand(0, 3);

define("_FREECAP_BASE", $mosConfig_absolute_path . "/components/com_securityimages/plugins/freecap/1.4/");

$dict_location = _FREECAP_BASE . "dict/english_freecap_words";

$font_locations = Array (
	_FREECAP_BASE . "fonts/ht_freecap_font1.gdf",
	_FREECAP_BASE . "fonts/ht_freecap_font2.gdf",
	_FREECAP_BASE . "fonts/ht_freecap_font3.gdf",
	_FREECAP_BASE . "fonts/ht_freecap_font4.gdf",
	_FREECAP_BASE . "fonts/ht_freecap_font5.gdf"
);

$bg_images = Array (
	_FREECAP_BASE . "bkg/ht_freecap_im1.jpg",
	_FREECAP_BASE . "bkg/ht_freecap_im2.jpg",
	_FREECAP_BASE . "bkg/ht_freecap_im3.jpg",
	_FREECAP_BASE . "bkg/ht_freecap_im4.jpg",
	_FREECAP_BASE . "bkg/ht_freecap_im5.jpg"
);
...
for ($i = 0; $i < sizeof($font_locations); $i++) {
	...
	$font_widths[$i] = ord($c_wid {
		8 }); //+ord($c_wid{9})+ord($c_wid{10})+ord($c_wid{11});
	fclose($handle);
}
...
if (empty ($_SESSION['freecap_attempts'])) {
	...
	if ($_SESSION['freecap_attempts'] > $max_attempts) {
		...
		ImageString($im, 5, 15, 20, "Max retry reached!", $red);
...
if ($use_dict == 1) {
	...
	for ($i = 0; $i < $wordlen; $i++) {
		...
		if ($rand_func (0, 4) >= 2 && $i != 0) {
			$word .= $vowels {
				$rand_func (0, strlen($vowels) - 1)
				};
		} else {
			$word .= $consonants {
				$rand_func (0, strlen($consonants) - 1)
				};
		}
	}
}
...
for ($i = 0; $i < strlen($word); $i++) {
	...
	ImageString($im2, $font, $word_start_x + ($font_widths[$j] * $i), $word_start_y, $word {
		$i }, $text_colour2);
}
// use last pixelwidth
...
// so that's cool.
$tag_str = ""; //"freeCap v1.4 - puremango.co.uk";
// for debug:
//$tag_str = "[".$word."]";
