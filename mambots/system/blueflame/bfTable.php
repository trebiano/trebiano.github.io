<?php
/**
* @version		$Id: bfTable.php 827 2007-06-12 18:03:41Z phil $
...
class bfTable extends bfDatabase
{
	/**
	...
	*/
	function load( $oid=null )
	{
		...
		{
			if (($name != $k) and ($name != "_db")
			and ($name != "_tbl") and ($name != "_tbl_key")
			and ($name != "_registry") and ($name != "_session")
			) {
		...
		$this->_tbl_key_SAVED = $this->_tbl_key;
		$oid_SAVED = $oid;
		if (!is_numeric($oid)){
			$this->_tbl_key = "LOWER(".$this->_tbl_key.")";
			$oid = "LOWER('".$oid."')";
		} else {
			$oid = "'$oid'";
		}
		...
		$this->_tbl_key = $this->_tbl_key_SAVED;
		$oid = $oid_SAVED;
