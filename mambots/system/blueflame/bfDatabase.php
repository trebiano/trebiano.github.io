<?php
/**
* @version		$Id: bfDatabase.php 827 2007-06-12 18:03:41Z phil $
...
class bfDatabase
{
	...
	/* whether joomfish is installed and active */
	var $_fish_enabled = false;

	function bfDatabase($host='localhost', $user, $pass, $db='', $table_prefix=''){
		$this->__construct($host, $user, $pass, $db, $table_prefix );
	}
	...
	*/
	function &getInstance( $driver='mysql', $host='localhost', $user, $pass, $db='', $table_prefix='' )
	{
		...
		if (empty($instances[$signature])) {
			bfLoad('bf'.ucwords($driver));
			$adapter = 'bfDatabase'.$driver;
			$instances[$signature] = new $adapter($host, $user, $pass, $db, $table_prefix);

			/**
			 * The fish compatibility
			 */
			if (bfCompat::isAdmin()===false){
				$filename = _BF_JPATH_BASE . DS . bfCompat::mambotsfoldername() . DS . 'system' . DS . 'jfdatabase.systembot.php';
				if (file_exists($filename)){
					$instances[$signature]->_fish_enabled = true;
				}
			}

			if ($instances[$signature]->_fish_enabled===true){
				$query = "select distinct reference_table from #__jf_content";
				$instances[$signature]->setQuery( $query );
				$instances[$signature]->_mlTableList = $instances[$signature]->loadResultArray();
				if( !$instances[$signature]->_mlTableList ){
					echo $instances[$signature]->getErrorMsg();
				}
			}
		}
	...
	function setQuery( $sql, $offset = 0, $limit = 0, $prefix='#__' )
	{
		...
		$this->_log[] = $this->replacePrefix( $sql, $prefix );
	...
	function _logTofile($str){
		if (_BF_SQL_LOG==false) return;
		$data = null;
		$unknownsFile = bfCompat::getAbsolutePath() . DS . 'components' . DS . $mainframe->get('component') . DS . 'sql' . DS . 'sql.log';
	...
	*/
	...
	function _isincache(){
		...
		if (ereg('SELECT ',$this->_sql)){
			/* cache */
			$limit = mosGetParam($_REQUEST,'limit', $this->_limit);
			$limitstart = mosGetParam($_REQUEST,'limitstart', $this->_offset);
			$this->cache_key = md5($this->_sql.$limit.$limitstart);
			$this->cache_data = $cache->api_get($this->cache_key,'sql');
	...
	function _savetocache($data){
		...
		if (ereg('SELECT ',$this->_sql)){
			...
			$cache->api_add($this->cache_key,$data,'sql');
		}
	}
}
?>