...
/**
 * @version $Id: bfInputFilter.php 827 2007-06-12 18:03:41Z phil $
 * @package bfFramework
...
class bfInputFilter
{
	...
	function & getInstance($tagsArray = array(), $attrArray = array(), $tagsMethod = 0, $attrMethod = 0, $xssAuto = 1)
	{
		...
		if (empty ($instances[$sig])) {
			$instances[$sig] = new bfInputFilter($tagsArray, $attrArray, $tagsMethod, $attrMethod, $xssAuto);
		}
	...
	function _cleanAttributes($attrSet)
	{
		...
		for ($i = 0; $i < count($attrSet); $i ++)
		{
			...
			if (bfInputFilter::checkAttribute($attrSubSet)) {
				continue;
			}
